...
description: Route bounded scout, worker, and validator assignments from a Codex lead through native Codex, Sol Medium native Codex, Claude Opus, Cursor/Grok, OpenCode/Grok, OpenCode/Qwen3.8 Max Preview, or OpenCode/Ollama execution lanes. Use when the user invokes $codex-subagent-routing, requests one of these Codex-led delegation policies, or combines lane selection with the goal prompt template.
---
...
- Sol Medium: read [codex-sol-medium.md](references/codex-sol-medium.md);
- Claude Opus through Claude CLI: read [claude-opus.md](references/claude-opus.md);
...
Apply the universal formal-return boundary to every review cycle. Let the
initial review assignment return, have the lead disposition its complete
report, let one correction assignment return, have the lead inspect that
coherent delta, and only then begin bounded delta revalidation. Keep at most one
review-cycle assignment active at a time.
...
A formal return is the only normal phase-transition boundary for every
assignment. An intermediate message, partial stream item, quiet observation
tick, user-facing update, or liveness result never completes the assignment and
never authorizes the lead to disposition or forward partial work, start a
dependent assignment, or reactivate another retained context. If an
intermediate message explicitly requests required input or reports that the
assignment cannot continue within its current contract, send one focused
response without changing phase; otherwise record it and continue observing.
...
| The assignment emits an intermediate message | Apply the universal intermediate-message rule above, then continue observing. |
| New evidence invalidates the contract, authorized scope, or lane eligibility | Steer or stop the affected assignment at that boundary. |
| The assignment formally returns | Inspect the complete result and apply the continuation rule below. |
| The harness reports an error or configured deadline, loses the recorded session or process, or supplies lane-specific evidence of frozen execution | Recover the assignment under the selected lane while preserving its role and freshness requirements. |
...
After a formal return, have the lead inspect and disposition the complete
result. Continue the recorded context only for a bounded follow-up permitted by
the assignment's role, route, freshness requirement, and still-current
contract. When those conditions do not hold, close it, keep the work in the
lead, or start a correctly configured fresh assignment. Never use continuation
to repair an invalid initial packet whose contract requires fresh context.
