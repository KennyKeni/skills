...
Apply the main skill's event loop. Observe through mailbox waits of roughly
one minute; when the loop permits a health check, inspect the active-agent
list once. Leave the subagent's worktree untouched during supervision.
