...
Apply the main skill's event loop. Observe through mailbox waits with a timeout
of at least one minute, using longer waits when the expected duration warrants
them; when the loop permits a health check, inspect the active-agent list once.
Leave the subagent's worktree untouched during supervision.
