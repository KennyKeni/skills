---
name: keni-improve-codebase-architecture
...
- **Naming a deepened module after a concept not in the local context docs?** Add the term to the relevant local context file — same discipline as `/keni-grill-with-docs` (see [CONTEXT-FORMAT.md](../keni-grill-with-docs/CONTEXT-FORMAT.md)). Create `.local/context/CONTEXT.md` lazily if no local context exists.
- **Sharpening a fuzzy term during the conversation?** Update the relevant local context file right there.
- **User rejects the candidate with a load-bearing reason?** Offer an ADR, framed as: _"Want me to record this as an ADR so future architecture reviews don't re-suggest it?"_ Only offer when the reason would actually be needed by a future explorer to avoid re-suggesting the same thing — skip ephemeral reasons ("not worth it right now") and self-evident ones. See [ADR-FORMAT.md](../keni-grill-with-docs/ADR-FORMAT.md).
