...
The issue tracker should have been provided in `.local/agents/issue-tracker.md` — run `/keni-setup-matt-pocock-skills` if it is missing. Read and follow that file's command conventions for every operation. Issues and PRDs live in the configured issue tracker, not local issue files.
