...
export class NonInteractiveSetupError extends Error {
  public constructor() {
    super("Interactive setup requires a TTY. Run groundwork setup from an interactive terminal.");
...
export async function runSetup(
  ...
  try {
    ...
    const members = plan.index.members ?? [];
    if (plan.index.mode === "fleet" && members.length > 0 && result.index === "created") {
      if (await driver.confirmMemberSetup(members)) {
        for (const member of members) {
          // Each member run is its own independent transaction: own discovery,
          // prompts, lock, and atomic publish. A cancelled member skips only
          // that member.
          await runSetup({ cwd: join(discovery.repositoryRoot, member) }, dependencies);
        }
      }
    }
    return result;
