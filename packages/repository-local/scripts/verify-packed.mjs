...
const temporaryRoot = await mkdtemp(join(tmpdir(), "repository-local-pack-"));
...
try {
  ...
  const help = run("npx", ["--no-install", "repository-local", "--help"], consumer);
  if (!help.includes("repository-local setup")) {
  ...
  const nonInteractive = runExpectFailure(
    "npx",
    ["--no-install", "repository-local", "setup", "--cwd", nonInteractiveRoot],
    consumer,
  );
  ...
  const installedManifest = JSON.parse(
    await readFile(join(consumer, "node_modules", "@kennykeni", "repository-local", "package.json"), "utf8"),
  );
  if (installedManifest.version !== sourceManifest.version) {
    throw new Error("Installed version did not match.");
  }
  const installedRoot = join(consumer, "node_modules", "@kennykeni", "repository-local");
  const installedFiles = await readdir(join(installedRoot, "templates"));
  const expectedTemplates = ["architecture.md", "development.md", "github.md"];
  if (JSON.stringify(installedFiles.sort()) !== JSON.stringify(expectedTemplates)) {
