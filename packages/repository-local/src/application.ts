...
import type { AnyRenderedConcern, RenderedPlan } from "./model.js";
import { renderIndexEntries, type IndexEntry } from "./rendering.js";

const lockName = ".repository-local.setup.lock";
const recoveryName = ".repository-local.setup-recovery.json";
const markerName = ".repository-local-install.json";
...
export type ApplyHooks = {
  readonly beforeDestinationClaim?: (concern: AnyRenderedConcern) => void | Promise<void>;
...
export class SetupLockedError extends Error {
  public constructor() {
    super("Another repository-local setup process holds the repository reservation.");
...
function reportCleanupLeak(hooks: ApplyHooks, path: string, reason: string): void {
  ...
  if (hooks.onCleanupLeak === undefined) {
    process.emitWarning(`Preserved temporary path ${path}: ${reason}`, { code: "REPOSITORY_LOCAL_CLEANUP_LEAK" });
  }
}
...
async function readRecovery(repositoryRoot: string): Promise<RecoveryRecord | undefined> {
  try {
    ...
    throw new Error("The repository-local recovery journal is invalid or unreadable.", { cause: error });
  }
}
...
async function stageConcern(
  ...
  const stagingFolder = await mkdtemp(join(destinationParent, `.repository-local-stage-${concern.id}-`));
  ...
  try {
    await Promise.all(
      [...concern.renderedFiles.map((file) =>
        writeFile(join(stagingFolder, file.name), file.contents, { encoding: "utf8", flag: "wx" }),
...
async function writeIndex(
  ...
  const stagingRoot = await mkdtemp(join(indexParent, ".repository-local-index-"));
  ...
  try {
    await writeFile(stagedIndex, renderIndexEntries(plan.indexPath, concerns), {
      encoding: "utf8",
      flag: "wx",
    });
    try {
