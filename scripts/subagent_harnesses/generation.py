...
def mark_generated_outputs(
    outputs: dict[Path, str], *, generator: Path, relative_to: Path
) -> dict[Path, str]:
    generator_name = generator.relative_to(relative_to).as_posix()
    return {
        path: _add_generated_notice(content, path, generator_name)
...
def _add_generated_notice(content: str, path: Path, generator_name: str) -> str:
    message = (
        f"Generated by {generator_name}; DO NOT EDIT this file directly. "
        "Edit the generator inputs and regenerate."
    )
    ...
    if content.startswith("---\n"):
        ...
        if frontmatter_end == -1:
            raise ValueError(f"unterminated frontmatter in generated file: {path}")
        insertion_point = frontmatter_end + len("\n---\n")
        remainder = content[insertion_point:].removeprefix("\n")
        return (
            f"{content[:insertion_point]}{notice}\n\n"
            f"{remainder}"
        )
    return f"{notice}\n\n{content}"
