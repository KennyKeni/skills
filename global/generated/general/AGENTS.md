...
When the user mentions Groundwork as a tool or CLI, they mean the `groundwork`
executable published by the `@kennykeni/groundwork` npm package. Run its
interactive repository setup with:

```sh
pnpm dlx @kennykeni/groundwork setup
```

Pass `--cwd /path/to/repository` when setup should target a repository other
